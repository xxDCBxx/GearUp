...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($request_id) {
                ...
                try {
                    if ($tx_type === 'market') {
                        ...
                        if (mysqli_num_rows($check) == 0) {
                            throw new Exception("Revert blocked: the buyer no longer has the received item.");
                        }
        ...
        elseif ($type === 'topup') {
            if ($action === 'approve') {
                $rs = mysqli_prepare($link, "SELECT t.user_id, t.amount, u.credits, u.email, u.name FROM topup_requests t JOIN users u ON u.id = t.user_id WHERE t.id = ? AND t.status = 'pending' AND t.email_verified = 1");
                mysqli_stmt_bind_param($rs, "i", $request_id);
                mysqli_stmt_execute($rs);
                $req = mysqli_fetch_assoc(mysqli_stmt_get_result($rs));
                mysqli_stmt_close($rs);
                ...
                if ($req) {
                    ...
                    try {
                        ...
                        $bal_res = mysqli_query($link, "SELECT credits FROM users WHERE id = $uid");
                        $new_bal = mysqli_fetch_row($bal_res)[0];

                        mysqli_query($link, "UPDATE topup_requests SET status = 'approved', reviewed_by = $me, reviewed_at = NOW() WHERE id = $request_id");
                        mysqli_commit($link);

                        $receipt_html = generate_receipt_email_body(
                            "Top-up Approved",
                            [['name' => 'Balance Top-up', 'price' => number_format($amount, 2) . ' USD']],
                            [
                                'Total' => number_format($amount, 2) . ' USD',
                                'Initial Balance' => '$' . number_format($req['credits'], 2),
                        ...
                        send_smtp_email($req['email'], "Top-up Approved - GEARUP!", $receipt_html, EMAIL_SMTP_FROM, EMAIL_SMTP_FROM_NAME, true);
